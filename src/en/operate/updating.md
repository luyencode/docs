...
>
> ⏱ ~15–30 min (longer if images must be rebuilt) · 👤 Operators · 🔑 SSH + permission to run `docker` and `git` on the server
...
- [ ] You have [backed up the database](/en/operate/operations#backup) (ideally `problems/` and `media/` too).
- [ ] You picked a quiet time with no contest running.
- [ ] You have SSH access and can run `docker compose` and `git` in `lcoj-docker/dmoj/`.
- [ ] You know which lcoj-site branch your install follows (the examples on this page use `prod/luyencode`).
- [ ] You know what a *migration* is (a database schema change generated by Django). See the [Glossary](/en/start/glossary).

The process at a glance:

```mermaid
flowchart TD
  A["Back up the database"] --> B["Record the current commit (OLD)"]
  B --> C["git pull outer repo + update repo/"]
  C --> D{"Dependencies or Dockerfiles changed?"}
  D -- Yes --> E["Rebuild base, then site, celery, bridged, wsevent"]
  D -- No --> F["docker compose up -d"]
  E --> F
  F --> G["./scripts/migrate + ./scripts/copy_static"]
  G --> H["Restart site, celery, bridged, wsevent"]
  H --> I{"Checks pass?"}
  I -- Yes --> J["Done"]
  I -- No --> K["Roll back to OLD, restore DB if needed"]
```
...
- `.gitmodules` has no `branch` entry, so `git submodule update --remote` follows lcoj-site's default branch (`master`), **not** `prod/luyencode`.
- The commands on this page follow lcoj-site's **`prod/luyencode`** branch. If you follow a different branch, substitute its name.
...
If you're on a detached HEAD and want to follow `prod/luyencode`:
...
| Symptom | Fix |
|---|---|
| `git pull` inside `repo/` fails because you're not on a branch | `repo/` is in detached HEAD. Run `git -C repo checkout prod/luyencode`, then pull again (see [Submodules and "detached HEAD"](#submodules-and-detached-head)) |
| `ModuleNotFoundError` after updating | New dependencies weren't installed: rebuild `base` first, then the other images ([Rebuilding](#rebuild)) |
...
| The site is badly broken and can't be fixed quickly | Follow [Rolling back](#rolling-back) below |
...
- [Operating LCOJ](/en/operate/operations): logs, clearing the cache, regular backups.
- [Environment Variables](/en/operate/environment): when a new version adds variables to `environment/*.env.example`.
- [Management Commands](/en/reference/management-commands): `./scripts/manage.py` commands you may need after an update.
